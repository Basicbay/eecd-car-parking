...
export default function CustomerPaymentPage() {
  ...
  // Payment states
  const [selectedMethod, setSelectedMethod] = useState<"promptpay" | "card" | "linepay">("promptpay")
  ...
  const getDurationText = () => {
    ...
    const durationHrs = (now - vehicle.checkInTime.getTime()) / (1000 * 60 * 60)
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#090A0C] text-white flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-xs text-muted-foreground">กำลังดึงข้อมูลบัตรจอดรถ...</p>
      </div>
    )
  }
      ...
      <header className="border-b border-[#22262F] bg-[#121418]/60 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-md mx-auto px-4 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xs font-bold tracking-wider uppercase text-white">EECD Car Parking</h1>
          </div>
          <span className="text-[10px] text-muted-foreground font-semibold bg-white/5 border border-white/10 px-2 py-0.5 rounded-full">
            Customer Portal
          </span>
        </div>
      </header>
          ...
          <>
            ...
            <div className="bg-[#121418]/90 border border-[#22262F] rounded-2xl shadow-xl p-5 space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[9px] font-bold text-primary tracking-wider uppercase">บัตรจอดรถดิจิทัล</span>
                  <h2 className="text-sm font-extrabold text-white mt-0.5">ข้อมูลการใช้บริการ</h2>
                </div>
                <div className="text-right">
                  <span className="text-[9px] font-mono text-muted-foreground">TICKET ID</span>
                  <p className="text-xs font-mono font-bold text-white uppercase">{vehicle.id}</p>
                </div>
              </div>
              ...
              <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-black/40 border border-[#22262F]">
                {vehicle.imageUrl ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground gap-2">
                    ...
                    <span className="text-[10px]">ไม่พบบันทึกรูปภาพจากกล้อง</span>
                  </div>
                )}
                {/* Status Indicator over image */}
                <div className="absolute top-3 right-3">
                  {vehicle.status === "Parked" && (
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold bg-primary text-[#090A0C] px-2.5 py-0.5 rounded-full shadow-md">
                      <span className="h-1 w-1 rounded-full bg-[#090A0C] animate-ping" />
                      กำลังจอดอยู่
                    </span>
                  )}
                  {vehicle.status === "Paid" && (
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold bg-green-500 text-white px-2.5 py-0.5 rounded-full shadow-md">
                      ชำระเงินแล้ว
                    </span>
                  )}
                  {vehicle.status === "Exited" && (
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold bg-[#22262F] text-muted-foreground px-2.5 py-0.5 rounded-full shadow-md">
                      ออกจากลานจอดแล้ว
                    </span>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3.5 bg-black/20 p-3.5 rounded-xl border border-[#22262F]/40 text-xs">
                <div className="space-y-0.5">
                  <span className="text-[9px] text-muted-foreground font-semibold uppercase">ทะเบียนรถ</span>
                  <p className="font-bold text-white text-sm">{vehicle.plate}</p>
                  <p className="text-[10px] text-muted-foreground">{vehicle.province}</p>
                </div>

                <div className="space-y-0.5">
                  <span className="text-[9px] text-muted-foreground font-semibold uppercase">ช่องจอดรถ</span>
                ...
                <div className="space-y-0.5 col-span-2 border-t border-[#22262F]/50 pt-2.5">
                  <span className="text-[9px] text-muted-foreground font-semibold uppercase">เวลาเช็คอินเข้าจอด</span>
                  <p className="font-semibold text-white">
                    {vehicle.checkInTime.toLocaleDateString("th-TH", { day: '2-digit', month: 'short', year: 'numeric' })}
                    {" • "}
                    {vehicle.checkInTime.toLocaleTimeString("th-TH", { hour: '2-digit', minute: '2-digit' })} น.
                  </p>
                </div>

                <div className="space-y-0.5 pt-1">
                  <span className="text-[9px] text-muted-foreground font-semibold uppercase">ระยะเวลาจอดรวม</span>
                ...
                <div className="space-y-0.5 pt-1">
                  <span className="text-[9px] text-muted-foreground font-semibold uppercase">ค่าจอดรถสะสม</span>
            ...
            {vehicle.status === "Parked" ? (
              // Payment Section
              <div className="bg-[#121418]/90 border border-[#22262F] rounded-2xl shadow-xl p-5 space-y-4">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">ชำระเงินผ่านช่องทางออนไลน์</h3>
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 flex justify-between items-center">
                  <span className="text-xs font-semibold text-muted-foreground">ยอดเงินที่ต้องชำระ</span>
                  <span className="text-2xl font-black text-primary">฿{vehicle.fee}</span>
                </div>

                {vehicle.fee === 0 ? (
                  // Free parking check-out button
                  <div className="space-y-3 pt-2">
                    <p className="text-[10px] text-muted-foreground text-center">ขณะนี้ยอดชำระของคุณคือ 0 บาท สามารถกดปุ่มยืนยันการเช็คเอาท์เพื่อเสร็จสิ้นกระบวนการได้ทันที</p>
                    <button
                      onClick={handlePay}
                      disabled={processing}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      >
                        ...
                        <span className="text-[9px] font-bold">บัตรเครดิต</span>
                      </button>
                      ...
                      >
                        <span className="text-[10px] font-black uppercase tracking-tighter">Line Pay</span>
                        <span className="text-[9px] font-bold">LINE Pay</span>
                      </button>
                    </div>
                    ...
                    <div className="bg-black/30 border border-[#22262F] p-4 rounded-xl min-h-[160px] flex flex-col justify-center">
                      {selectedMethod === "promptpay" && (
                        <div className="text-center space-y-3.5">
                          ...
                          <div className="flex items-center justify-center gap-1 bg-[#0f2c59] py-1 px-3.5 rounded-lg border border-[#1d4c8a]/50 w-fit mx-auto">
                            <span className="text-[8px] font-black tracking-wide text-white uppercase">Prompt</span>
                            <span className="text-[8px] font-black tracking-wide text-[#a5d1ff] uppercase">Pay</span>
                          </div>
                          ...
                          <div className="p-2.5 border border-[#22262F] bg-white inline-block rounded-xl mx-auto shadow-inner">
                            <MockQRCode value={`PROMPTPAY-EECD-CARPARK-FEE-${vehicle.fee}`} />
                          </div>
                          <p className="text-[9px] text-muted-foreground max-w-[240px] mx-auto leading-normal">
                            บันทึกภาพ QR Code หรือแสกนตรงเพื่อชำระผ่านแอปพลิเคชันธนาคารของคุณได้ทันที
                          </p>
                        </div>
                      )}
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 text-xs">
                            <input
                              ...
                              onChange={(e) => setCreditCard({ ...creditCard, number: e.target.value.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim() })}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <input
                                ...
                                onChange={(e) => setCreditCard({ ...creditCard, expiry: e.target.value.replace(/\D/g, '').replace(/(.{2})/g, '$1/').replace(/\/$/, '') })}
                                className="w-full bg-[#121418] border border-[#22262F] focus:border-primary/80 focus:ring-1 focus:ring-primary/80 h-9 px-3 rounded-lg text-xs outline-none transition-all"
                              />
                              <input
                                type="password"
                      ...
                      {selectedMethod === "linepay" && (
                        <div className="text-center py-4 space-y-3">
                          ...
                          <div>
                            <p className="text-xs font-bold text-white">Rabbit LINE Pay</p>
                            <p className="text-[9px] text-muted-foreground mt-0.5 leading-normal">
                              ชำระเงินสะดวกผ่านบัญชี Rabbit LINE Pay หรือกระเป๋าเงิน LINE Wallet ของคุณ
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <button
                      onClick={handlePay}
                      disabled={processing || (selectedMethod === "card" && (!creditCard.number || !creditCard.expiry || !creditCard.cvv))}
              ...
              <div className="bg-[#121418]/90 border border-[#22262F] rounded-2xl shadow-xl p-5 text-center space-y-5 animate-fadeIn">
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-bold text-green-400 tracking-wider uppercase">ทำรายการสำเร็จ</span>
                  <h3 className="text-base font-extrabold text-white">ชำระเงินเสร็จสมบูรณ์แล้ว!</h3>
                  <p className="text-[10px] text-muted-foreground max-w-[250px] mx-auto leading-relaxed">
                    ระบบได้รับยอดชำระของคุณเรียบร้อยแล้ว ประตูทางออกลานจอดจะเปิดโดยอัตโนมัติเมื่อขับเข้าใกล้ที่กั้นภายใน 15 นาที
                  </p>
                </div>
                ...
                <div className="bg-primary/5 border border-primary/20 p-4 rounded-xl space-y-2.5 text-center relative overflow-hidden group">
                  ...
                  <span className="inline-flex items-center gap-1.5 text-[9px] font-bold text-primary uppercase tracking-wider">
                  ...
                  <div className="flex gap-2 max-w-[260px] mx-auto">
                    ...
                    <button
                      onClick={() => copyWifiCode(`EECD-WIFI-${vehicle.id.toUpperCase()}`)}
                  ...
                  <p className="text-[8px] text-muted-foreground leading-normal max-w-[200px] mx-auto">
                    SSID: <b>EECD-FreeWiFi</b> (ใช้งานได้ 1 ชั่วโมงหลังจากเริ่มเชื่อมต่ออุปกรณ์เครื่องแรก)
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="bg-[#121418]/90 border border-[#22262F] rounded-2xl shadow-xl p-6 text-center space-y-5 animate-fadeIn">
            ...
            <div className="space-y-1.5">
              <h2 className="text-base font-extrabold text-white">
                {id === "search" ? "ค้นหาข้อมูลค่าบริการจอดรถ" : "ไม่พบบันทึกบัตรจอดรถ"}
              </h2>
            ...
            {searchError && (
              <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg text-[10px] font-semibold">
                {searchError}
              </div>
            )}
            ...
            <form onSubmit={handleSearchVehicle} className="space-y-3.5 text-left">
              <div className="space-y-1.5">
